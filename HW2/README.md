<p align="center">
  <img src="./images/logo.png" alt="國立金門大學校務資訊系統風格橫幅" width="820">
</p>
...
![通識課程加選表格與模擬校務選單](./screenshots/p1.png)

### 02 · 我的課表：選課後同步排入逐節課表

![已選課程同步進入我的課表](./screenshots/p2.png)
...
![教師課表與課程查詢結果](./screenshots/p3.png)

#### 04 · 加選衝堂防呆提示

![課程衝堂時顯示醒目提示彈窗](./screenshots/p4.png)

#### 05 · 超修／酌減學分申請條件提示

![酌減學分申請資格提示](./screenshots/p5.png)
...
| 檔案 | 用途 |
| --- | --- |
| **index.html**、**css/styles.css** | 頁面結構、經典校務風格與響應式版型 |
| **js/app.js** | 選單導覽、查詢、加退選、課表與申請互動 |
| **js/local-generator.js** | AI 不可用時的本地模擬資料生成 |
| **js/ai-client.js**、**js/ai-config.js** | 前端 AI 請求、模型設定及資料格式 |
| **worker/cloudflare-worker.js** | Cloudflare Worker 代理、Gemini 請求與模擬紀錄整理 |
| **images/** | **logo.png**、**code.png**、**online.png** 為展示版介面圖片 |
| **screenshots/** | **p1.png–p5.png** 為本 README 的功能截圖 |

## 🛠️ 自行部署

1. Fork 或複製本儲存庫，將 **GitHub Pages** 指向 `main` 分支的根目錄；純靜態前端不需要打包。
2. 若要啟用 AI，將 **worker/cloudflare-worker.js** 部署為 Worker，設定 Secret **GEMINI_API_KEY**，以及允許網站來源的變數 **ALLOWED_ORIGIN**（例如 `https://你的帳號.github.io`，請填完整 Origin，**不含**儲存庫路徑）。不要把 API 金鑰放進 GitHub Pages、README 或前端 JS。
3. 在 **js/ai-config.js** 設定公開 Worker URL，並視需要調整模型設定；目前使用 **gemini-2.5-flash**。
